...
$(document).on("click", ".chat-message__edit", function () {
  ...
  let hasFiles = parent.find(".chat-message__files .chat__file").length > 0;
  let firstFileName = parent.find(".chat-message__files .chat__file .chat__file__link p").first().text().trim();
  ...
  let repliedContent = '';
  if (hasText && !hasFiles) {
    repliedContent = messageText;
  } else if (hasFiles && !hasText) {
    repliedContent = firstFileName;
  } else if (hasFiles && hasText) {
    repliedContent = `
    ...
    <div class="chat__edit">
      <div class="chat__footer__container">
        ...
        <div class="chat__answered">
          ...
          ${repliedContent ? `<p>${repliedContent}</p>` : ''}
        </div>
      </div>
