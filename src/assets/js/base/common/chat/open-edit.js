...
$(document).on("click", ".chat-message__edit", function () {
  let parent = $(this).closest(".chat-message__author");
  let message = parent.closest(".chat-message");
  
  // Очищаем предыдущие состояния
  $(".chat__edit, .chat__reply, .chat__bottom .chat__files").remove();
  $('.chat__form').removeClass('replied').find("textarea[name='comment']").val("");
  
  let messageText = parent.find(".chat-message__author__wrapper > p").contents()
    .filter(function() { 
      return this.nodeType === 3; // Только текстовые узлы
    }).text().trim();
    
  let hasText = messageText.length > 0;
  let hasFiles = parent.find(".chat__files .chat__file").length > 0;
  let firstFileName = hasFiles ? parent.find(".chat__files .chat__file .chat__file__link p").first().text().trim() : '';
  ...
  // Формируем контент блока редактирования
  let repliedContent = '';
  if (hasText && !hasFiles) {
    repliedContent = `
      <p>
  ...
  $(".chat__footer").prepend(editTemplate);
  $(".chat__footer textarea[name='comment']").val(messageText).focus();
